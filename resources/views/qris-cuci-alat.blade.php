<x-layouts.app title="Pembayaran Cuci Alat - {{ config('app.name') }}" description="Pembayaran layanan cuci alat gunung.">
        ...
        <div class="max-w-md w-full mx-auto px-4">
            ...
            <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-200/60 overflow-hidden">
                ...
                <div class="bg-primary p-6 text-center text-white relative overflow-hidden">
                    ...
                    <h1 class="text-xl font-bold relative z-10 font-['Hanken_Grotesk']">Pembayaran Cuci Alat</h1>
                ...
                <div class="p-8">
                    ...
                    <div class="mb-6 pb-6 border-b border-dashed border-slate-200">
                        ...
                        <div class="flex flex-col gap-3">
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="font-semibold text-slate-800">{{ $order['nama'] }}</span>
                            </div>
                            <div class="flex justify-between text-sm">
                                <span class="text-slate-500">Paket</span>
                                <span class="font-semibold text-slate-800">{{ $order['paket_name'] }}</span>
                            </div>

                            {{-- Daftar Barang --}}
                            <div class="mt-2 flex flex-col gap-2">
                                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider font-['JetBrains_Mono']">Daftar Barang</span>
                                @foreach ($order['items'] as $i => $item)
                                    <div class="bg-slate-50 rounded-xl p-3 flex flex-col gap-1 border border-slate-100">
                                        <div class="flex justify-between text-sm">
                                            <span class="text-slate-500">Barang #{{ $i + 1 }}</span>
                                            <span class="font-semibold text-slate-800 capitalize">{{ str_replace('_', ' ', $item['jenis_alat']) }}</span>
                                        </div>
                                        <div class="flex justify-between text-sm">
                                            <span class="text-slate-500">Jumlah</span>
                                            <span class="font-semibold text-slate-800">{{ $item['jumlah'] }} item</span>
                                        </div>
                                        @if($item['catatan'] && $item['catatan'] !== '-')
                                            <div class="flex justify-between text-sm gap-4">
                                                <span class="text-slate-500 shrink-0">Catatan</span>
                                                <span class="text-slate-700 text-right">{{ $item['catatan'] }}</span>
                                            </div>
                                        @endif
                                    </div>
                                @endforeach
                    ...
                    <div class="flex flex-col items-center justify-center mb-8">
                        <div class="w-48 h-48 bg-white border-2 border-slate-200 rounded-2xl p-4 flex items-center justify-center relative mb-4 shadow-sm shadow-slate-100">
                            ...
                            @else
                                <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data={{ urlencode('CUCI_ALAT_' . $order['paket_id'] . '_' . $order['nama']) }}" alt="QRIS Barcode" class="w-full h-full object-contain">
                    ...
                    <div class="bg-slate-50 rounded-2xl p-4 flex items-center justify-between mb-6">
                        ...
                        <span class="text-xl font-black text-secondary-500 font-['Hanken_Grotesk']">
                            {{ $order['paket_price'] }}
                            <span class="text-sm font-normal text-slate-400">{{ $order['paket_unit'] }}</span>
                        </span>
                    </div>
                    ...
                    @php
                        $itemLines = '';
                        foreach ($order['items'] as $i => $item) {
                            $jenisLabel = str_replace('_', ' ', $item['jenis_alat']);
                            $catatan = ($item['catatan'] && $item['catatan'] !== '-') ? $item['catatan'] : 'Tidak ada catatan';
                            $itemLines .= "%0A*Barang " . ($i + 1) . "*%0A"
                                . "- Jenis: {$jenisLabel}%0A"
                        ...
                        $waMessage = "Halo Admin Project Gunung!%0A%0ASaya ingin mengonfirmasi pembayaran cuci alat:%0A%0A"
                            ...
                            . "%0ADaftar Barang:"
                            . $itemLines
                            . "%0A%0ABerikut saya lampirkan bukti pembayarannya.";
