...
<div
    class="relative w-full h-[100dvh] min-h-[620px] overflow-hidden"
    x-data="{
        active: 0,
        total: {{ count($slides) }},
        interval: null,
...
>
    ...
    {{-- Konten Utama (Teks + Button Sewa Sekarang) --}}
    <div class="relative z-10 h-full max-w-[1280px] mx-auto px-5 sm:px-6 lg:px-12 flex flex-col justify-start pt-24 sm:justify-center sm:pt-0 pb-36 sm:pb-0">
        <div class="w-full md:w-1/2 flex flex-col items-start gap-2.5 sm:gap-6">
            ...
            <p class="text-stone-300 text-xs sm:text-base lg:text-lg leading-relaxed max-w-[500px] line-clamp-3 sm:line-clamp-none">
                {{ __('home.hero_desc') }}
            </p>

            <div class="flex flex-row items-center gap-4 mt-1 sm:mt-2">
                <a href="/rental" class="px-5 py-2.5 sm:px-8 sm:py-4 bg-kuning hover:bg-kuning/90 rounded-lg flex justify-center items-center gap-2 transition-all active:scale-95 shadow-md">
    ...
    {{-- Info Panel Cuaca Kanan Bawah --}}
    <div class="absolute bottom-3 sm:bottom-16 lg:bottom-12 left-3 right-3 lg:left-auto lg:right-12 z-20">
        <div class="bg-neutral-900/85 rounded-xl sm:rounded-2xl border border-stone-300/20 backdrop-blur-md p-2.5 sm:p-4 lg:p-6 flex items-center justify-between lg:justify-start gap-1.5 sm:gap-6 lg:gap-10 shadow-2xl overflow-x-auto">
            @foreach ($slides as $index => $slide)
                @php
                    $def = $defaults[$index] ?? $defaults[0];
                @endphp
                <div
                    class="flex items-center justify-between w-full gap-2 sm:gap-6 lg:gap-10"
                    @if ($index > 0) x-cloak @endif
                    x-show="active === {{ $index }}"
                    ...
                    x-transition:enter-end="opacity-100"
                >
                    <div class="flex flex-col items-center min-w-[60px] sm:min-w-[70px] lg:min-w-[100px]">
                        ...
                        <span class="text-zinc-200 text-[11px] sm:text-sm lg:text-lg font-semibold text-center leading-tight">{{ $slide['name'] }}</span>
                    </div>

                    <div class="w-px h-7 sm:h-10 lg:h-12 bg-neutral-600/50 shrink-0"></div>
                    ...
                    <div class="flex flex-col items-center shrink-0">
                        ...
                        <span class="text-zinc-200 text-sm sm:text-xl lg:text-3xl font-semibold">{{ $slide['elevation'] }}</span>
                    </div>

                    <div class="w-px h-7 sm:h-10 lg:h-12 bg-neutral-600/50 shrink-0"></div>

                    <div class="flex flex-col items-center min-w-[40px] sm:min-w-[50px] lg:min-w-[72px] shrink-0">
                        ...
                        <span
                            class="text-zinc-200 text-sm sm:text-xl lg:text-3xl font-semibold tabular-nums"
                    ...
                    <div class="flex flex-col items-center min-w-[40px] sm:min-w-[50px] lg:min-w-[72px] shrink-0">
                        ...
                        {{-- Cerah --}}
                        <svg x-show="weatherType({{ $index }}) === 'sunny'" @if($def['weather'] !== 'sunny') x-cloak @endif class="w-4 h-4 sm:w-6 sm:h-6 lg:w-8 lg:h-8 text-yellow-300 mt-0.5" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        {{-- Hujan --}}
                        <svg x-show="weatherType({{ $index }}) === 'rainy'" @if($def['weather'] !== 'rainy') x-cloak @endif class="w-4 h-4 sm:w-6 sm:h-6 lg:w-8 lg:h-8 text-blue-300 mt-0.5" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M6.995 12c2.893 0 5.234-2.379 5.234-5.31A5.36 5.36 0 0 0 12 5.051a6 6 0 1 1-5.005 6.949z"/>
                        </svg>

                        {{-- Salju --}}
                        <svg x-show="weatherType({{ $index }}) === 'snowy'" @if($def['weather'] !== 'snowy') x-cloak @endif class="w-4 h-4 sm:w-6 sm:h-6 lg:w-8 lg:h-8 text-sky-200 mt-0.5" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M6.995 12c2.893 0 5.234-2.379 5.234-5.31A5.36 5.36 0 0 0 12 5.051a6 6 0 1 1-5.005 6.949z"/>
                        </svg>

                        {{-- Berkabut --}}
                        <svg x-show="weatherType({{ $index }}) === 'foggy'" @if($def['weather'] !== 'foggy') x-cloak @endif class="w-4 h-4 sm:w-6 sm:h-6 lg:w-8 lg:h-8 text-stone-400 mt-0.5" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M6.995 12c2.893 0 5.234-2.379 5.234-5.31A5.36 5.36 0 0 0 12 5.051a6 6 0 1 1-5.005 6.949z"/>
                        </svg>

                        {{-- Berawan (default) --}}
                        <svg x-show="weatherType({{ $index }}) === 'cloudy'" @if($def['weather'] !== 'cloudy') x-cloak @endif class="w-4 h-4 sm:w-6 sm:h-6 lg:w-8 lg:h-8 text-stone-300 mt-0.5" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M6.995 12c2.893 0 5.234-2.379 5.234-5.31A5.36 5.36 0 0 0 12 5.051a6 6 0 1 1-5.005 6.949z"/>
                        </svg>

                        <span
                            class="text-stone-400 text-[8px] sm:text-[10px] font-['JetBrains_Mono'] mt-0.5 uppercase tracking-wide"
