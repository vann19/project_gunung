...
<div
    class="relative w-full h-[100dvh] lg:h-screen lg:overflow-hidden"
    x-data="{
        active: 0,
        total: {{ count($slides) }},
        interval: null,
...
>
    ...
    {{-- Konten Utama (Text, Button di Mobile) --}}
    <div class="relative z-10 h-full max-w-[1280px] mx-auto px-5 sm:px-6 lg:px-12 flex flex-col justify-center pb-32 lg:pb-0">
    ...
    {{-- Widget Cuaca (Mobile & Desktop) --}}
    <div class="absolute bottom-28 lg:bottom-12 left-4 right-4 lg:left-auto lg:right-12 z-20">
        <div class="bg-neutral-800/60 rounded-2xl border border-stone-300/10 backdrop-blur-md p-4 lg:p-6 flex items-center justify-between lg:justify-start gap-2 lg:gap-10 shadow-2xl overflow-x-auto">
            @foreach ($slides as $index => $slide)
                ...
                @endphp
                <div
                    class="flex items-center justify-between w-full lg:w-auto lg:gap-10"
                    @if ($index > 0) x-cloak @endif
                    x-show="active === {{ $index }}"
    ...
    {{-- Indikator Carousel --}}
    <div class="absolute bottom-20 lg:bottom-12 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2">
        @foreach ($slides as $index => $slide)
            <button
                type="button"
