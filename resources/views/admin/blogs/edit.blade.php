<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Edit Artikel Blog</h1>
                <p class="text-sm text-slate-500 mt-1">Perbarui konten artikel blog ini.</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto pb-12">
        <form action="{{ route('admin.blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 sm:p-8 space-y-6">
                ...
                <div>
                    ...
                    @if($blog->image)
                        <div class="mb-3 w-40 rounded-xl overflow-hidden border border-slate-200">
                            <img src="{{ $blog->image }}" alt="Thumbnail" class="w-full h-auto object-cover">
                        </div>
                    @endif
                ...
                <div>
                    ...
                    <textarea name="content" rows="12" required
                              class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all duration-200">{{ old('content', $blog->content) }}</textarea>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <label for="is_published" class="text-sm font-medium text-slate-700 cursor-pointer">Publikasikan (Tampil di Website)</label>
                </div>
            </div>
