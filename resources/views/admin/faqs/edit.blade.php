<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Edit FAQ</h1>
                <p class="text-sm text-slate-500 mt-1">Perbarui pertanyaan atau jawaban FAQ ini.</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto pb-12">
        <form action="{{ route('admin.faqs.update', $faq->id) }}" method="POST" class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 sm:p-8 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="answer" rows="6" required
                              class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all duration-200">{{ old('answer', $faq->answer) }}</textarea>
