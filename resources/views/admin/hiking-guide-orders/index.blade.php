                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ketua Tim</th>
                            ...
                            @forelse($orders as $order)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-900 font-bold">
                                        {{ $order->ketua_tim }}<br>
                                        <span class="text-xs font-normal">WA: {{ $order->whatsapp }}</span>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        {{ \Carbon\Carbon::parse($order->tanggal_pendakian)->format('d M Y') }}<br>
                                        {{ $order->durasi_hari }} Hari
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        <div class="mb-2">
                                            <strong class="text-xs text-gray-900 block mb-1">Ketua:</strong>
                                            @if($order->foto_ktp)
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <form action="{{ route('admin.hiking-guide-orders.status', $order->id) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <form action="{{ route('admin.hiking-guide-orders.destroy', $order->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus pesanan ini?');">
                                            @csrf
                                            @method('DELETE')
